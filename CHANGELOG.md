...
- Discovery Closing Document as a **PowerPoint**: the Closing document tab exports the saved document as a deck (client version and a version with the consultant notes); one heading per slide, long tables split, appendices and bibliography left to the annex.
- The closing document is now **two documents**: the deck narrative that is presented, and an **annex** with the decision analysis (criteria, options with pros and cons, why rejected options were rejected), the capability analysis, appendices and a bibliography with quotes and checked dates.
- **Merkle Shopify reference chapters** (`discovery/docs/reference/`, mirrored into bundled code by `npm run reference:chapters`): plans, Markets, Managed Markets, payments and multi-currency, Liquid vs Hydrogen, B2B, migration, AI and agentic commerce — verified against official Shopify documentation with a date, selected per engagement by code and appended to the annex, so Claude cites them instead of rewriting them.
- Capability map deepened: each requirement carries the client's own wording, why that resolution level and not a cheaper one (enforced for app, theme and custom rows), and the documented limits or licence implications.
- Merkle footer on every page: wordmark and the dentsu line, Legal Terms, Privacy Policy and Your Privacy Choices linking to merkle.com, Instagram, YouTube and LinkedIn, and a line saying this is an internal pilot tool.
- Header menu underlines in red on hover and keeps the underline on the current section, as on merkle.com.
- Closing document tab reorganised: the status and the single action (Generate / Update, with the number of answers that changed) sit at the top, the changes and the files below, and everything else folded away. The action card turns green when the document is up to date.
- Starting the generation takes one click: the Closing document tab opens Claude with the instruction already written (or copies it), the connector publishes the same instruction as prompt templates (draft, redraft, pre-fill from documents), and the tab refreshes itself so the new version appears without reloading. The tool still cannot start Claude on its own — MCP is client-driven and an API key was ruled out — but the consultant no longer has to know what to type.
- Solution design completed in the deck: an integration slide per system (who owns the data, how it moves, what happens when it fails), the data model, the migration (what moves, what does not, cut-over and rollback), non-functional requirements with targets and verification, open decisions with owner and date, out of scope, the operating model after go-live, the cost to run, AI shopping channels and a closing conclusion slide. The engine demands each one when the engagement calls for it.
- The deck is a consulting argument, not a questionnaire summary: for each business problem a slide with what it costs today and how Shopify solves it; for each requirement a slide with **what Shopify does as standard, what we decide, why not a cheaper level, what it covers and what it does not**; an app case per app (why native cannot do it, what it covers, what it does not, cost, alternatives rejected); a gaps slide for everything Shopify cannot cover; and the solution architecture. The engine refuses a deck that skips them.
- Deck preview in the browser: see every slide before downloading the PowerPoint.
- The deck is built from fixed slide templates that Claude fills (title, agenda, statement, decision with options and the chosen one marked, KPI cards, risk register, roadmap, configuration split, tables, investment, next steps): code owns every layout, so the deck reads like consulting work instead of prose on slides. The annex is rendered as its own PowerPoint too.
- Every generated file carries its version: `<client>-discovery-closing-document-v1.0.pptx`, `-annex-v1.0.pptx` and the Markdown equivalents; each save increments (v1.0, v1.1 …) and previous versions are kept.
- PowerPoint download fixed: pptxgenjs is bundled into the server build (`ssr.noExternal`), because the deployed function cannot resolve a bare import that the build leaves external.
- Merkle look and feel: navy header with the red brand mark, Merkle's colour scale from merkle.com (checked 2026-09-17), Figtree as a free stand-in for the licensed Proxima Nova, pill buttons, red active tab, cards and focus states; every colour pair meets WCAG AA contrast.
- The closing document knows when the answers moved under it: saving snapshots the answers, and the Closing document tab then shows **up to date** or **N answers changed since**, lists which questions moved with old and new values, and offers a ready-made line for Claude that names them. The Claude connector returns the same status, so Claude redrafts with the changes in hand (owner decision: track what changed, redraft in full, keep versions).
- **Every question now explains itself** (297 of 297): why a Shopify discovery asks it, what the answer changes in the engagement (scope gate, larger-engagement trigger, exit rule or app signal, derived from the rules), the documented limits, options with pros and cons where it is a real either/or, and official sources. Shown to the consultant in the web app and printed in the consultant guide; a test keeps new questions from shipping without it.
- Question-bank review notes from that pass: `discovery/docs/question-bank-review.md` (wording, duplicates, numbering gaps and questions whose answers drive nothing).
- Questions teach: a `teach` block (why it matters, options with pros and cons, limits, sources) is shown to the consultant in the web app and in the consultant guide, so a consultant who knows commerce but not Shopify can run the conversation and explain the trade-off. Section 7.7 carries it; the plan- and architecture-deciding questions follow.
- Questionnaire: new subsection **7.7 AI & agentic commerce** (12 questions on agentic channel enrolment, checkout inside the assistant, US-buyer eligibility, supplemental terms, customer-data sharing, Shopify Catalog readiness and mapping, AI crawler policy, Knowledge Base, UCP and MCP plans, Sidekick/Magic/semantic search) with a new `ai` section in the engagement contract; automated flows, tag manager and custom-event tracking are now asked in a standard interview.
- Lead Consultant manual (`/manual`, linked in the header, readable without sign-in): the flow from engagement and consent, pre-filling answers from RFPs in the Claude Project, review and interview, to generating, reviewing and sharing the Discovery Closing Document, with troubleshooting.
- The Discovery Closing Document is generated in the Claude Project by default (strongest model, web search on): Project instructions now make Claude research official Shopify sources before drafting and fix rejected gaps by adding evidence; `/architect` in Claude Code stays the advanced path for complex engagements.
- Sourced solution architecture in the Discovery Closing Document (ADR 0017): architecture decisions with options, integration architecture per system, data model, non-functional requirements and a risk register; every Shopify fact cites an official Shopify source and every client fact its question ids, enforced by code in the connector, Claude Code and API mode; deck data, deck prompt (consulting standard, answer first, references) and a new `architecture.md` artefact; `/architect` Claude Code skill (connector + Shopify Dev MCP, strongest model) recommended on the Closing document tab.
