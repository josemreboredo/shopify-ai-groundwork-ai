...
|---|---|---|---|---|---|
| 11.1 | A Shopify Plus feature is required (B2B, Checkout Extensibility, Launchpad, expansion stores) but the target plan is not Plus | STOP | Confirm Shopify Plus or remove the feature from scope | Q1.2.3, Q3.1.4, Q4.2.1, Q6.2.1 | |
| 11.2 | B2B requires RFQ / quote or custom negotiated pricing per buyer | STOP | Architecture review of the quote workflow (Shopify Plus B2B prerequisite) | Q6.2.6 | |
...
| 11.12 | ERP or PIM with no existing Shopify connector and no iPaaS *(proposed)* | FLAG | Separate integration scoping track (T3/T4) | Q8.1.1 | |
| 11.13 | fulfilment_locations > 2 AND complex_routing is true *(proposed)* | FLAG | Multi-location inventory scoping (T3) | Q5.1.3, Q5.1.4 | |
| 11.14 | Migration with significant SEO equity or complex historical data *(proposed)* | FLAG | Dedicated migration scoping track — not combined with the store build sprint | Q8.2.3, Q8.2.4, Q8.2.5 | |
| 11.15 | Weeks from kick-off (delivery.kickoff_date, else meta.created_at) to target go-live are fewer than the offer's minimum duration_weeks *(proposed)* | FLAG | Re-scope to an MVP-first delivery before any sprint begins | Q10.1.1 | |
| 11.16 | No single decision-maker, or budget approval authority is unclear *(proposed)* | STOP | Pause engagement until commercial and governance clarity is confirmed | Q10.2.2, Q10.2.3 | |
