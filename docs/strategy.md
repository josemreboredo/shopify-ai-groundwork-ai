...
the scope and the delivery team gives the final figure. The packs are delivered by an
offshore team with QA in it; contingency and services such as Design are added in
the proposal.
...
| `+Marketplaces` — each marketplace listed through Shopify Marketplace Connect: products mapped to its categories and attributes, prices and listing rules per marketplace, and orders and stock tested both ways before it goes live. | +1–4 wk | +CHF 6.3k–25.2k |
...
| `+Hydrogen` — the storefront built as a React front end on Hydrogen instead of a Liquid theme, on top of the full template set it needs designed: the front end over a Liquid build of the same templates, and the back end a theme never needs — every query, hook and event wired to Shopify’s APIs. | +5–7 wk | +CHF 31.5k–44.1k |
| `+Accessibility audit` — a conformance audit against every success criterion of the agreed standard: every template and state of the storefront tested by hand with screen readers on desktop and mobile, the app blocks included; what fails fixed and re-tested; and the result written up as an accessibility conformance report the client can hand to a buyer or a regulator. Checkout is covered by Shopify’s own report. | +1–2 wk | +CHF 6.3k–12.6k |
...
| `+SKU (very large)` — 50,000 SKUs and above: catalogue operations in their own right — staged loads, a rehearsal run, and a data owner on the client side, with Search & Discovery set up for it | +3–5 wk | +CHF 18.9k–31.5k |
| `+AI content` — the catalogue’s content written or enriched with AI: a brand-voice guide and the prompts built from it, the attributes mapped to category metafields, generation across every product, a sample checked by people and approved by the client, and one product CSV import. A quarter of a build week per 1,000 SKUs, and three quarters at the least, which covers setting it up. | +0.75–12.5 wk | +CHF 4.7k–78.8k |
| `+Search (native)` — search & Discovery taken to its limits: the filter set designed against the product data that backs it, boosts and synonyms per collection, and a merchandiser who can change them afterwards | +0.5–1 wk | +CHF 3.2k–6.3k |
| `+Search (app)` — past a documented Shopify limit, so a third-party search app: indexing and re-indexing, the sync that keeps it true, and the merchandising rules moved into it with an owner | +1.5–2.5 wk | +CHF 9.5k–15.8k |
| `+SEO (standard)` — the redirect map authored and imported, metadata and canonicals carried across, sitemap and indexation checked through cut-over | +0.5–1 wk | +CHF 3.2k–6.3k |
| `+SEO (large)` — a redirect estate of 10,000 URLs or more, or rankings the business runs on: the map built from a crawl rather than a pattern, verified on staging, and indexation watched after launch | +1.5–2.5 wk | +CHF 9.5k–15.8k |
| `+Subscriptions (standard)` — Shopify Subscriptions: selling plans, the storefront widget, the customer portal and the billing states the support team has to recognise | +0.5–1 wk | +CHF 3.2k–6.3k |
| `+Subscriptions (advanced)` — a third-party subscription app, or prepaid, build-a-box, B2B or international plans — and live contracts carried across without the customer re-entering a card, which is where these migrations fail | +1.5–2.5 wk | +CHF 9.5k–15.8k |
| `+Checkout (post-purchase)` — UI extensions on the Thank you and Order status pages — order tracking, surveys, referrals, an app block after the sale. Available on every plan except Starter, so this needs no Plus | +0.5–1 wk | +CHF 3.2k–6.3k |
| `+Checkout (in-checkout)` — UI extensions and custom fields on the information, shipping and payment steps. Shopify Plus only — on any other plan this is not a scope decision, it is a plan decision | +1–2 wk | +CHF 6.3k–12.6k |
| `+Checkout (functions)` — backend logic as Shopify Functions — validation, delivery or payment customisation, order rules: a deployed app with its own tests and release path, not a setting | +1.5–3 wk | +CHF 9.5k–18.9k |
| `+Promotions (standard)` — one rule Shopify’s own discounts cannot express — tiered or volume discounts for consumers — as a discount Function with its rule table, unit tests on cart fixtures, thresholds a marketer changes without a deploy, and the storefront messaging that explains it. | +1–1.5 wk | +CHF 6.3k–9.5k |
| `+Promotions (advanced)` — stacking rules of the business’s own across discounts: every rule written down against what the native combinations already cover, the remainder as a discount Function with its tests, configuration a marketer owns, and the cart and product-page messaging for each promotion. | +2–3 wk | +CHF 12.6k–18.9k |
| `+Loyalty (standard)` — a loyalty app for what Shopify’s own rewards cannot do: earning rules for purchases and actions, rewards as discounts or store credit, VIP tiers, referrals and member early access configured to the client’s programme; its launcher, product and account blocks added through the theme editor and branded; every text and the programme terms translated; tested with test customers and handed over with an owner. | +1–1.5 wk | +CHF 6.3k–9.5k |
| `+Loyalty (advanced)` — the standard work, and the programme past the storefront — half a week more: points, tiers and rewards synced to the email platform or CRM as profile properties its flows can use, or earned and spent in the client’s own shops on Shopify POS, with the staff shown how. | +1.5–2 wk | +CHF 9.5k–12.6k |
| `+Agentic (standard)` — past the AI every pack includes: the product data that lives in metafields, metaobjects or titles mapped into Shopify Catalog as structured fields, and an AI crawler policy set in robots.txt.liquid, checked on the live domain before and after. | +0.5–1 wk | +CHF 3.2k–6.3k |
| `+Agentic (advanced)` — the store’s own shopping assistant, or the store connected to agent platforms, on top of the AI every pack includes: built on the store’s Storefront MCP endpoint — catalogue search, cart and the store’s policies — with the conversation designed, guardrails and an evaluation set the brand agreed, consent before any customer data reaches a model, and the model’s running cost named as the client’s, like an app’s fee. | +4–6 wk | +CHF 25.2k–37.8k |
| `+Analytics (standard)` — customer events, the destinations wired through them and consent respected on each — measurement that agrees with Shopify's own numbers | +0.5–1 wk | +CHF 3.2k–6.3k |
| `+Analytics (advanced)` — server-side tagging or a third-party consent platform: consent state carried to every destination, capture points beyond checkout, and the reconciliation that proves the numbers still line up | +1.5–2.5 wk | +CHF 9.5k–15.8k |
| `+Messaging (standard)` — an email platform in place of Shopify Messaging: its Shopify app connected, customers, orders, catalogue and consent synced both ways, sign-up forms through its app embed, a master email template built to the brand, and up to five lifecycle flows rebuilt from its templates in the storefront’s languages — each tested with a test profile, and Shopify’s own automations switched off so nothing sends twice. The copy is the client’s. | +0.5–1 wk | +CHF 3.2k–6.3k |
| `+Messaging (advanced)` — the standard work, and a platform past the lifecycle flows — half a week more: more than five flows, or SMS or WhatsApp sent through something other than Shopify Messaging, with its own consent capture and the sender registration each country asks for. | +1–1.5 wk | +CHF 6.3k–9.5k |
| `+Support (standard)` — one of the two: written SOPs, or a handover into a retainer that is not the Grow retainer — the runbooks and the owner named against each, past the launch-day training every offer carries | +0.5–1 wk | +CHF 3.2k–6.3k |
| `+Support (extended)` — both: a documented operating model the client’s team runs the store from, handed into a retainer that has to receive it, with a training programme behind it | +1–2 wk | +CHF 6.3k–12.6k |
| `+Returns (standard)` — a returns or post-purchase app past Shopify’s own returns: configured to the client’s policy, labels or QR drop-off with the carriers the client uses, the return portal and tracking page branded to the storefront, delivery updates on the channels asked for, warranty claims where they are in scope — tested on real orders and handed over with an owner. | +1–1.5 wk | +CHF 6.3k–9.5k |
| `+Returns (advanced)` — the standard work, and the app reconciled with the systems around it — half a week to a week more: items inspected in the warehouse before the refund is released, and every return, refund and cancellation passed to finance with a daily check that the two agree. The connection to the ERP itself is the integration add-on. | +1.5–2.5 wk | +CHF 9.5k–15.8k |
| `+Retail` — each location selling with Shopify POS: hardware, inventory, staff and the in-store test pass | +1–5 wk | +CHF 6.3k–31.5k |
| `+Migration (light)` — WooCommerce or Shopify-to-Shopify: catalogue, customers and order history. The redirect estate is the SEO continuity gate, not this one. | +1–1.5 wk | +CHF 6.3k–9.5k |
| `+Migration (medium)` — Shopware or BigCommerce: data model differences to map, order history, customer and price-list structures. Redirects are priced by the SEO continuity gate. | +2.5–4 wk | +CHF 15.8k–25.2k |
| `+Migration (heavy)` — Magento, Salesforce Commerce Cloud or a custom platform: a bespoke data model to map and integrations to re-point. Redirects are priced by the SEO continuity gate. | +4–5.5 wk | +CHF 25.2k–34.7k |
| `+MultiCurrency` — price lists, rounding, payout and reconciliation per currency — priced on its own only where one market sells in several currencies; a further market carries its own | +0.5 wk | +CHF 3.2k |
| `+Languages` — each language beyond the third: translation of products, collections, policies, URL handles, theme and app strings — and again on every release | +0.5–2.5 wk | +CHF 3.2k–15.8k |
<!-- /generated:modifiers -->
