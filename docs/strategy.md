...
| `+Agentic (standard)` — the AI channels taken on deliberately: the position decided and the supplemental terms accepted by whoever holds the authority, the customer data direct checkout shares reviewed against the privacy notice, the channel settings, the catalogue mapped from custom fields where it lives there, the answers agents give curated in the Knowledge Base app, and an AI crawler policy set and tested in robots.txt.liquid. | +1–1.5 wk | +CHF 6.3k–9.5k |
| `+Agentic (advanced)` — the store’s own shopping assistant, or the store connected to agent platforms, on top of the standard work: built on the store’s Storefront MCP endpoint — catalogue search, cart and the store’s policies — with the conversation designed, guardrails and an evaluation set the brand agreed, consent before any customer data reaches a model, and the model’s running cost named as the client’s, like an app’s fee. | +4–6 wk | +CHF 25.2k–37.8k |
