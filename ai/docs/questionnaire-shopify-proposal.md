...
- **Scope:** `discovery/schema/question-bank.json` 1.0.0 (211 questions), `contracts/engagement.schema.json`, `discovery/schema/offering.json` 1.1.0, `discovery/agents/discovery/app-signals.js`
...
Add one optional, validated block to each question in `discovery/schema/question-bank.json`. It says which part of Shopify answers the question, on which plan, and where that is documented:
...
| `apps` | array of app handles (the apps.shopify.com slug) | Shortlist seeds. They point to a single `discovery/schema/apps.json` registry (name, developer, URL, category, last_checked), so an app is described once |
...
- `approach.js` and `app-signals.js` take the shortlist from `shopify.apps` and `discovery/schema/apps.json`, not from prose. This removes the app names that are hard-coded in section intros and help texts (e.g. "Loop, AfterShip, parcelLab, Narvar").
...
1. **A test that runs on every commit** (`discovery/tests/unit/question-bank-shopify.test.js`):
   ...
   - every `apps` handle exists in `discovery/schema/apps.json`
...
2. **A release check at each Shopify Edition** (winter, spring and summer; Spring '26 was published in 2026). This is a Track C methodology task owned by the Architect:
   ...
   - For the content check, the Shopify AI Toolkit plugin (the `shopify-dev` doc search skill in Claude Code) or the Shopify Dev MCP answers each `native[].feature` and `plan_note` against current docs. The output is a diff report in `discovery/docs/shopify-verification-<edition>.md`.
...
| O8 | **App registry governance:** who approves apps in `discovery/schema/apps.json` (partner relationships, security review of app data access under the PII gate), and how often? | The shortlist becomes data the engine recommends. |
