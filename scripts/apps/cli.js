...
/**
 * @file cli.js
 * @description App Store registry (schema/apps.json). Apps enter as `proposed`;
...
export const REGISTRY = path.join(REPO_ROOT, 'schema', 'apps.json');
 ...
 */
export function approveApp(registry, { handle, by, today, engagement, note }) {
  const app = registry.apps.find((a) => a.handle === handle);
  if (!app) throw new Error(`Unknown app handle "${handle}" — add it to schema/apps.json first`);
