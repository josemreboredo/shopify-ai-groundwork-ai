...
| 5. Exit rules 11.1–11.22, Shopify plan benchmark, merged with LLM candidates | `exits.js`, `plan.js` | no |
| 6. Approach — capability map, app shortlist, assumptions, phases (GO, or Larger Engagement) | `approach.js` | yes (structured output) |
| 7. Validation against `contracts/engagement.schema.json` | `engine.js` | no |
...
`discovery/tests/unit/discovery-rules.test.js` (gates, offer, exit rules against the golden fixtures) and
`discovery/tests/unit/discovery-engine.test.js` (full pipeline with recorded LLM responses — no network).
Threat model: [`discovery/docs/architecture/discovery-engine-threat-model.md`](../../docs/architecture/discovery-engine-threat-model.md).
