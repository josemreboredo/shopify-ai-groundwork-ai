...
6. Discovery Closing Document (when the Lead Consultant asks): the recommended way is /architect in Claude Code, which researches Shopify's documentation with the Shopify Dev MCP. Here: call prepare_closing_document. If step is "approach", research every Shopify fact in the official documentation (web search on help.shopify.com, shopify.dev, apps.shopify.com), then draft the approach as one JSON object following the instructions and approach_schema — sourced architecture decisions, integration architecture, data model, non-functional requirements and risk register — and save it with save_approach (fix and retry any errors it returns by adding the missing evidence). Then write the complete document in Markdown from deck_xml following the returned instructions, and save it with save_closing_document. Tell the Lead Consultant to download it from the engagement's Closing document tab.
